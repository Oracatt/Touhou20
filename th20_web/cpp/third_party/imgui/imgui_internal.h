...
namespace ImGui
{
    ...
    IMGUI_API void          BeginColumns(const char* str_id, int count, ImGuiOldColumnFlags flags = 0, float default_offset[] = nullptr); // setup number of columns. use an identifier to distinguish multiple column sets. close with EndColumns().
