...
void ImGui::BeginColumns(const char* str_id, int columns_count, ImGuiOldColumnFlags flags, float default_offset[])
{
    ImGuiContext& g = *GImGui;
    ImGuiWindow* window = GetCurrentWindow();
    ...
    if (columns->IsFirstFrame && default_offset) {
        for (int n = 0; n < columns_count; n++) {
            SetColumnOffset(n, default_offset[n]);
        }
    }
...
void ImGui::Columns(int columns_count, const char* id, bool border, bool resize, float default_offset[])
{
    ImGuiWindow* window = GetCurrentWindow();
    IM_ASSERT(columns_count >= 1);

    ImGuiOldColumnFlags flags = (border ? (resize ? 0 : ImGuiColumnsFlags_NoResize) : ImGuiColumnsFlags_NoBorder);
    ...
    if (columns_count != 1)
        BeginColumns(id, columns_count, flags, default_offset);
}
